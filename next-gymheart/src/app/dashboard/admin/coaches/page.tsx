...
function messageText(updated?: string, error?: string) {
  ...
  if (error === "self_status")
    return "Không thể tự vô hiệu hóa tài khoản đang đăng nhập.";
...
export default async function AdminCoachesPage({
  ...
  const { data } = await supabase
    .from("users")
    .select(
      "id, full_name, email, phone, avatar_url, bio, role, requested_role, specialization, years_of_experience, certification, is_active, created_at, pt_request_note",
    )
    .eq("role", "coach")
    .order("full_name");
  ...
  const coachesWithCv = await Promise.all(
    coaches.map(async (coach) => {
      const noteSignedUrl = await getSignedCvUrl(
        supabase,
        coach.pt_request_note,
      );
  ...
  return (
    <section className="rounded-2xl border border-pink-100 bg-white p-6 shadow-sm">
      <div className="mb-5 flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-2 text-sm text-muted">
            Quản lý hồ sơ, chuyên môn, trạng thái và tài khoản HLV.
          </p>
        </div>
      ...
      {message ? (
        <div
          className={`mb-5 rounded-xl px-4 py-3 text-sm font-bold ${params.error ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}
        >
          {message}
        </div>
      ) : null}
      ...
      <div className="space-y-4">
        {coachesWithCv.map((coach) => (
          <article
            className="rounded-2xl border border-pink-100 bg-background p-5 hover:border-primary"
            key={coach.id}
          >
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2 py-1 text-xs font-black ${coach.is_active ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-600"}`}
                    >
                  ...
                  <p className="mt-2 line-clamp-2 text-sm text-muted">
                    {coach.bio || "Chưa có phần giới thiệu."}
                  </p>
                </div>
              </div>
