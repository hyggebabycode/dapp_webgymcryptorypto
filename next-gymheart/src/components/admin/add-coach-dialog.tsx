...
import { addCoachAction } from "@/lib/admin/actions";
    ...
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4">
          <form action={addCoachAction} className="max-h-[92vh] w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl" encType="multipart/form-data">
            <div className="flex items-center justify-between border-b border-pink-100 px-6 py-5">
              <div>
                <h2 className="text-2xl font-black">Thêm huấn luyện viên</h2>
                <p className="mt-1 text-sm text-muted">Thông tin này sẽ dùng cho danh sách HLV và quản lý lớp.</p>
              </div>
              <button
                aria-label="Đóng"
            ...
            <div className="max-h-[calc(92vh-150px)] overflow-y-auto p-6">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <label className="md:col-span-2">
                  <span className="mb-2 block text-sm font-black">Địa chỉ / khu vực làm việc</span>
                  <input className="h-11 w-full rounded-lg border border-pink-100 px-4 outline-none focus:border-primary" name="address" placeholder="VD: Quận 1, TP.HCM hoặc chi nhánh phụ trách" />
                </label>
                <label>
                  <span className="mb-2 block text-sm font-black">Chuyên môn</span>
                  <input className="h-11 w-full rounded-lg border border-pink-100 px-4 outline-none focus:border-primary" name="specialization" placeholder="Fitness, Yoga, Boxing..." />
                </label>
                <label>
                ...
                <label className="md:col-span-2">
                  ...
                  <textarea className="min-h-24 w-full rounded-lg border border-pink-100 px-4 py-3 outline-none focus:border-primary" name="bio" />
                </label>
              </div>
            </div>
