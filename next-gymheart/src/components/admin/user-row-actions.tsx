    ...
    <div className="grid w-[236px] grid-cols-2 gap-2 text-xs font-black">
      ...
      <form
        ...
        onSubmit={(event) => {
          if (
            !window.confirm(
              `${user.is_active ? "Vô hiệu hóa" : "Kích hoạt"} tài khoản ${user.full_name}?`,
            )
          ) {
      ...
      {!isAdmin ? (
        <form
          ...
          onSubmit={(event) => {
            if (
              !window.confirm(
                `Lưu trữ tài khoản ${user.full_name}? Tài khoản sẽ không đăng nhập được nữa.`,
              )
            ) {
              event.preventDefault();
            }
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4">
          <div className="w-full max-w-2xl rounded-3xl bg-white shadow-2xl">
            ...
            <div className="grid gap-4 p-6 text-sm md:grid-cols-2">
              ...
              <Info
                label="Trạng thái"
                value={user.is_active ? "Active" : "Đã khóa"}
              />
              <Info
                label="Số điện thoại"
              ...
              <Info
                label="Kinh nghiệm"
                value={`${user.years_of_experience ?? 0} năm`}
              />
              <Info
                className="md:col-span-2"
                label="Chứng chỉ"
                value={user.certification || "Chưa cập nhật"}
              />
              <Info
                className="md:col-span-2"
                label="Giới thiệu"
                ...
                <div className="rounded-xl bg-background p-4 md:col-span-2">
                  ...
                  {user.cvSignedUrl ? (
                    ...
                    <p className="mt-2 text-sm font-bold text-muted">
                      Ứng viên có gửi file: {user.cvLabel}. Chưa tạo được link
                      xem file.
                    </p>
                  ) : (
                    <p className="mt-2 text-sm font-bold text-muted">
            ...
            <div className="flex items-center justify-between border-b border-pink-100 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Cập nhật thông tin người dùng hoặc huấn luyện viên.
                </p>
              </div>
              <button
                aria-label="Đóng"
            ...
            <div className="max-h-[calc(92vh-150px)] overflow-y-auto p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field
                  ...
                  defaultValue={String(user.years_of_experience ?? 0)}
                  type="number"
                />
                <Field
                  className="md:col-span-2"
