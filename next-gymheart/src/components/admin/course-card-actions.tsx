...
function serializeLessons(course: CourseAdminRecord) {
  return (course.course_lessons || [])
    ...
    .sort(
      (left, right) =>
        Number(left.lesson_order || 0) - Number(right.lesson_order || 0),
    )
    .map((lesson) =>
      [lesson.title, lesson.content || "", lesson.objectives || ""].join(" | "),
    )
    .join("\n");
}
...
function serializeSchedules(course: CourseAdminRecord) {
  return (course.schedules || [])
    ...
    .sort((left, right) => {
      const dayCompare =
        Number(left.day_of_week || 0) - Number(right.day_of_week || 0);
...
export function CourseCardActions({
  ...
  course: CourseAdminRecord;
  coaches: CoachOption[];
}) {
  const [detailOpen, setDetailOpen] = useState(false);
  const [editOpen, setEditOpen] = useState(false);
  const enrollmentLabels = (course.enrollments || []).map((enrollment) => {
    const student = one(enrollment.users);
    ...
    return `${studentName}${studentEmail}${statusText}${paymentText}`;
  });

  return (
    <div className="mt-4 grid grid-cols-2 gap-2 sm:w-[236px]">
      <button
        className="h-9 w-[112px] rounded-lg border border-primary text-xs font-black text-primary hover:bg-primary hover:text-white"
      ...
      <button
        className="h-9 w-[112px] rounded-lg border border-blue-100 text-xs font-black text-blue-600 hover:bg-blue-50"
      ...
      <form action={toggleCourseStatusAction}>
        ...
        />
        <button
          className="h-9 w-[112px] rounded-lg border border-orange-100 text-xs font-black text-orange-600 hover:bg-orange-50"
          type="submit"
        >
      ...
      <form
        ...
        onSubmit={(event) => {
          if (
            !window.confirm(
              `Lưu trữ khóa học ${course.course_name}? Khóa sẽ được ẩn khỏi danh sách đăng ký mới.`,
            )
          ) {
      ...
      >
        <input name="course_id" type="hidden" value={course.id} />
        <button
          className="h-9 w-[112px] rounded-lg border border-red-100 text-xs font-black text-red-600 hover:bg-red-50"
          type="submit"
        >
          Lưu trữ
        </button>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/55 p-4">
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-pink-100 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  {formatBaseAsTest(course.price)} · {course.duration_weeks}{" "}
                  tuần
                </p>
              </div>
              <button
            ...
            <div className="space-y-4 p-6 text-sm">
              ...
              <Info
                label="Lịch học"
                value={course.schedule_description || "Chưa cập nhật"}
              />
              <Info
                label="Thời gian"
              ...
              <ListInfo
                ...
                values={(course.schedules || [])
                  ...
                  .map(
                    (schedule) =>
                      `${dayLabels[schedule.day_of_week] || schedule.day_of_week}: ${String(schedule.start_time).slice(0, 5)} - ${String(schedule.end_time).slice(0, 5)}${schedule.location ? ` · ${schedule.location}` : ""}${schedule.room_number ? ` · ${schedule.room_number}` : ""}`,
                  )}
              />
              <ListInfo label="Danh sách học viên" values={enrollmentLabels} />
          ...
          >
            <div className="flex items-center justify-between border-b border-pink-100 px-6 py-5">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">
                  Cập nhật nội dung hiển thị và thông tin quản lý khóa học.
                </p>
              </div>
              <button
                aria-label="Đóng"
            ...
            <div className="max-h-[calc(92vh-150px)] overflow-y-auto p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field
                  label="Học phí (TEST)"
                  name="price"
                  defaultValue={String(baseAmountToTest(course.price))}
                  min={0.0001}
                  required
                  step={0.0001}
                  type="number"
                ...
                <Field
                  ...
                  name="duration_weeks"
                  defaultValue={String(course.duration_weeks)}
                  min={1}
                  required
                  type="number"
                />
                <Field
                  label="Sức chứa"
                  name="max_students"
                  defaultValue={String(course.max_students)}
                  min={1}
                  type="number"
                />
                <label>
                  ...
                  <select
                    className="h-11 w-full rounded-lg border border-pink-100 px-4 outline-none focus:border-primary"
                    name="level"
                ...
                <label className="md:col-span-2">
                  ...
                  <input
                    className="w-full rounded-lg border border-pink-100 bg-white px-4 py-2 text-sm outline-none focus:border-primary"
                    name="image_file"
                    accept="image/*"
                    type="file"
                  />
                ...
                <Field
                  className="md:col-span-2"
                  label="Lịch học hiển thị"
                  name="schedule_description"
                  defaultValue={course.schedule_description || ""}
                />
                <label className="md:col-span-2">
                  <span className="mb-2 block text-sm font-black">
                    Khung giờ học
                  ...
                  <textarea
                    ...
                    placeholder={
                      "Mỗi dòng: Thứ | Giờ bắt đầu | Giờ kết thúc | Địa điểm | Phòng | Ghi chú\nThứ hai | 17:00 | 18:30 | Khu B | Phòng CrossFit | Training chuyên sâu"
                    }
                  />
                  <p className="mt-2 text-xs font-bold text-muted">
                    Các dòng này sẽ tạo lịch cụ thể theo từng ngày trong khoảng
                    ngày bắt đầu và kết thúc của khóa.
                  </p>
                </label>
                <label className="md:col-span-2">
                  <span className="mb-2 block text-sm font-black">Mô tả</span>
                  <textarea
                    className="min-h-24 w-full rounded-lg border border-pink-100 px-4 py-3 outline-none focus:border-primary"
                    name="description"
                ...
                <label>
                  ...
                  <textarea
                    className="min-h-28 w-full rounded-lg border border-pink-100 px-4 py-3 outline-none focus:border-primary"
                    name="benefits"
                ...
                <label className="md:col-span-2">
                  ...
                  />
                  <p className="mt-2 text-xs font-bold text-muted">
                    Danh sách này sẽ thay thế lộ trình hiện tại của khóa.
                  </p>
                </label>
              </div>
            </div>
            <div className="flex gap-3 border-t border-pink-100 bg-primary-soft p-6">
              <button
                className="h-11 flex-1 rounded-lg border border-primary bg-white font-black text-primary"
                onClick={() => setEditOpen(false)}
                type="button"
              >
                Hủy
              ...
              <button
                className="h-11 flex-1 rounded-lg bg-primary font-black text-white"
                type="submit"
              >
                Lưu thay đổi
              </button>
