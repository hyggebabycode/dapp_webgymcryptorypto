...
import { createWalletChallengeAction, loginWithMetaMaskAction } from "@/lib/auth/actions";
...
function getErrorMessage(error: unknown) {
  ...
  return "Không thể đăng nhập bằng MetaMask. Vui lòng thử lại.";
}
...
export function MetaMaskLoginButton() {
  ...
  async function loginWithWallet() {
    ...
    try {
      ...
      setSubmitting(true);
      setMessage(`Đã kết nối ${shortAddress(address)}. Đang đăng nhập...`);
  ...
  return (
    <form action={loginWithMetaMaskAction} className="space-y-2" ref={formRef}>
      ...
      >
        <WalletCards size={20} />
        {submitting ? "Đang đăng nhập..." : "Đăng nhập với MetaMask"}
      </button>
