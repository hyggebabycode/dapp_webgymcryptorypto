...
-- Sửa constraint bcrypt nếu database_setup.sql bản cũ từng tạo regex bị escape sai.
ALTER TABLE users
DROP CONSTRAINT IF EXISTS chk_users_password_hash_bcrypt;

ALTER TABLE users
ADD CONSTRAINT chk_users_password_hash_bcrypt
CHECK (password_hash ~ '^\$2[aby]\$[0-9]{2}\$.{53}$') NOT VALID;
